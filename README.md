...
along with the path to train it (`scripts/train_vimeo_stage1.py`,
`colab_train_stage1.ipynb`) and the harness to score it
([`scripts/benchmark_intra_gate.py`](outputs/benchmarks/intra_gate_baseline/README.md),
which reproduces the deployed codec's intra totals byte-for-byte). It is **not
trained and not deployed**: every number on this page still comes from the 593k
baseline. What would change that is the Vimeo-90k run, then the intra-only BD-rate
gate — against the current codec's **+176.2%** in
[`outputs/benchmarks/parity_intra/`](outputs/benchmarks/parity_intra/README.md).
